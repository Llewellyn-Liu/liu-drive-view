<template>
  <v-app>
    ...
    <v-bottom-navigation>
      ...
      <v-btn @click="test_connection">Show</v-btn>
      <v-btn @click="sc_fileView=!sc_fileView">ChangeFV</v-btn>
      <v-btn @click="sc_testConnection=!sc_testConnection">ChangeObj</v-btn>
      <v-btn @click="sc_testConnection=sc_fileView">ValObj</v-btn>
      <v-btn @click="test_sendStream">StreamTest</v-btn>
    </v-bottom-navigation>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      sc_galleryView: true, //sc_galleryView and sc_fileView show what Main section is displaying.
      sc_fileView: true,
  ...
  methods: {
    ...
    test_connection() {
      console.log("Gallery: " + this.sc_galleryView, "File: " + this.sc_fileView, "Obj:" + this.sc_testConnection)
    },

    test_sendStream() {
      const b = new Blob([JSON.stringify({"value1": "apple", "value2": "banana"})], {type: "application/json"});
      fetch("http://localhost:8080/test/form3", {
        method: "POST",
        body: b.stream(),
        duplex: "half",
      })
